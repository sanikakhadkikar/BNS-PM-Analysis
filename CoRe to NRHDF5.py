...
#File selection 
gw_core = h5py.File('/home/sanikawork/Desktop/Sathya/CoRe/THC_0001/R01/data.h5','r')
...
for l in range(l_max+1):
    for m in range(0,l+1,1):
        ...
        spline = romspline.ReducedOrderSpline(np.array(gw_core['rh_22'][keys[count]][:,8]),mag,verbose=False)
...
for l in range(l_max+1):
    for m in range(0,l+1,1):
        ...
        df.create_group(str_name)
        spline = romspline.ReducedOrderSpline(np.array(gw_core['rh_22'][keys[count]][:,8]),phase,verbose=False)
        df[str_name].create_dataset('X', data=spline.X)
        df[str_name].create_dataset('Y', data=spline.Y)
        df[str_name].create_dataset('deg', data=[5])
        df[str_name].create_dataset('errors', data=spline.errors)
